...
- 账户 key 以明文存储于 `$DSH_HOME/storages/accounts.json`，界面只显示掩码。请把 DSH 数据目录当作本地凭证库，保护好其访问权限。余额查询优先使用当前账户 key，无激活账户时回退到系统凭证库。
...
| 账户 | key 以明文存于 `$DSH_HOME/storages/accounts.json`（与 harness 自身凭证存储一致）；界面只显示掩码，切换会把所选 key 写入凭证库用于后续 LLM 计费。请像保护本地凭证库一样保护 DSH 数据目录。 |
