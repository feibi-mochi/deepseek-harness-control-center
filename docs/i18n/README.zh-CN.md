...
- **v4 峰谷计费时钟**——`v4-flash` 与 `v4-pro` 专用的 24 小时常驻环形钟，实时呈现当前时段（高峰/低谷半价优惠）、下一次切换倒计时与系统级桌面切换提醒。工作日高峰为北京时间 9:00–12:00、14:00–18:00；自 2026-08-23 起，周六和周日全天按低谷价计费。其他模型维持固定价格。
- **官方价格同步**——定期检查 DeepSeek 官方价格页，只有完整校验通过的价格表才会应用；官方页面不可用或结构变化时，会明确保留内置规则。
...
- 账户 key 在 `$DSH_HOME/storages/accounts.json` 中加密保存：Windows 使用当前用户 DPAPI，其他系统使用仅限所有者访问的 AES-GCM 密钥文件。界面只显示掩码。余额查询优先使用当前账户 key，无激活账户时回退到系统凭证库。
...
2. 选择横排或竖排，并用比例滑条在 100%–120% 之间调整大小。
...
| 账户 | key 在 `$DSH_HOME/storages/accounts.json` 中加密存储：Windows 使用当前用户 DPAPI，其他系统使用仅限所有者访问的 AES-GCM 密钥文件；界面只显示掩码，切换会把所选 key 写入凭证库用于后续 LLM 计费。请像保护本地凭证库一样保护 DSH 数据目录。 |
...
- 2026-04-24 起——v4-flash 1/2（缓存读 0.02）、v4-pro 3/6（缓存读 0.025）、v4-flash-vision-exp 1/2（缓存读 0.02）
- 2026-08-17 北京时间 00:00 起——v4 模型实行峰谷定价（高峰为北京时间 9:00–12:00 / 14:00–18:00；空闲时段价格为高峰的一半）：
  ...
  - v4-flash-vision-exp（空闲 / 高峰）：缓存读 0.05 / 0.10、输入 1.5 / 3、输出 4.5 / 9
