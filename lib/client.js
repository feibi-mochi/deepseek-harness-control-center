...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var css = [
      ...
      '.dshw_footRingNarrow .dshw_footRingMoney{display:flex;flex-wrap:wrap;column-gap:5px;row-gap:1px;overflow:visible;text-overflow:clip;white-space:normal}',
    ...
    function PeakRingFooter(props) {
      ...
      // or a fixed card width: the stock layout stays byte-for-byte compact
      // when it already fits, while constrained cards wrap complete values.
      useLayoutEffect(function () {
        ...
        function measure() {
          var hadNarrowClass = node.classList && node.classList.contains('dshw_footRingNarrow')
          if (hadNarrowClass) node.classList.remove('dshw_footRingNarrow')
          ...
          var overflowed = !!((money && money.scrollWidth > money.clientWidth + 1)
            || (countdown && countdown.scrollWidth > countdown.clientWidth + 1))
          if (hadNarrowClass) node.classList.add('dshw_footRingNarrow')
      ...
      }, [snapshot, isRail, isVertical, isFreeFloating, peakScale, showRecharge])
