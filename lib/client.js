...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function planLimitPercent(source, id) {
      ...
      return limit && Number.isFinite(limit.percentage) ? Math.round(limit.percentage) + '%' : '--'
    }
    ...
    function PlanUsagePanel(props) {
      ...
      var summary = data === null ? '正在读取套餐…'
        : data.configuredCount > 0
          ? data.availableCount + '/' + data.configuredCount + ' 个套餐可用' + (firstLimit && Number.isFinite(firstLimit.percentage) ? ' · 已用 ' + Math.round(firstLimit.percentage) + '%' : '')
          : '未检测到已配置套餐'
      ...
      var sourceRows = shown.map(function (source) {
        ...
        var limits = Array.isArray(source.limits) ? source.limits.map(function (limit) {
          var pct = Number.isFinite(limit.percentage) ? Math.min(100, Math.max(0, limit.percentage)) : null
          var fillClass = 'dshw_planBarFill' + (pct !== null && pct >= 90 ? ' high' : pct !== null && pct >= 75 ? ' warn' : '')
          ...
          return React.createElement('div', { key: limit.id, className: 'dshw_planLimit' },
            React.createElement('div', { className: 'dshw_planLimitTop' },
              ...
              React.createElement('span', null, pct === null ? '—' : Math.round(pct) + '%')),
            React.createElement('div', { className: 'dshw_planBar', role: 'progressbar', 'aria-label': source.name + ' ' + planLimitLabel(limit), 'aria-valuemin': 0, 'aria-valuemax': 100, 'aria-valuenow': pct === null ? undefined : Math.round(pct) },
              React.createElement('div', { className: fillClass, style: { width: (pct === null ? 0 : pct) + '%' } })),
            React.createElement('div', { className: 'dshw_planLimitMeta' },
              React.createElement('span', null, planWindowLabel(limit)),
              React.createElement('span', null, used !== null && total !== null ? used + ' / ' + total : reset ? '重置 ' + reset : '按官方额度统计')))
        }) : []
        ...
        React.createElement('div', { className: 'dshw_settingChoices' },
          ...
          React.createElement('div', { className: 'dshw_settingChoice' },
            React.createElement('span', { className: 'dshw_settingChoiceCopy' },
              ...
              React.createElement('span', null, '侧边栏底部 24h 峰谷时钟')),
    ...
    function WalletChip(props) {
      ...
      var showDeepSeek = showOfficial && activeProviderMode.kind === 'deepseek'
      ...
      var activePlanSource = activeProviderMode.kind === 'zai'
        ...
        : null
      var planTokenPercent = planLimitPercent(activePlanSource, 'tokens-5h')
      var planToolPercent = planLimitPercent(activePlanSource, 'tools-month')
      ...
      if (!showDeepSeek) chipClass += ' dshw_chipNoRecharge'
      ...
      if (activeProviderMode.kind === 'zai') {
        if (chipVertical) {
          ...
          chipTextParts.push(verticalMetric('plan-token', '5h', planTokenPercent))
          chipTextParts.push(verticalMetric('plan-tools', 'MCP', planToolPercent))
          ...
          chipTextParts.push(React.createElement('span', { key: 'plan-token' }, '5h ' + planTokenPercent))
          chipTextParts.push(React.createElement('span', { key: 'plan-tools' }, 'MCP ' + planToolPercent))
      ...
      var chip = React.createElement('span', {
        ...
        showDeepSeek ? React.createElement('button', {
          type: 'button',
      ...
      var panel = React.createElement('div', {
        ...
        style: panelStyle,
        role: 'dialog',
        'aria-label': activeProviderMode.kind === 'zai' ? 'Z.ai 套餐与用量明细' : activeProviderMode.kind === 'third' ? providerDisplayName(activeProviderMode) + ' 用量明细' : 'DeepSeek \u94b1\u5305\u660e\u7ec6',
        onClick: function (e) { e.stopPropagation() }
      },
        floatBtnRow,
        showDeepSeek ? React.createElement(React.Fragment, null,
        ...
        showThird ? React.createElement(React.Fragment, null,
          React.createElement('div', { className: 'dshw_title' }, activeProviderMode.kind === 'zai' ? 'Z.ai 本会话 Token' : activeProviderMode.kind === 'third' ? providerDisplayName(activeProviderMode) + ' 本会话 Token' : '\u7b2c\u4e09\u65b9\u5408\u8ba1'),
          thirdRows,
          React.createElement('div', { className: 'dshw_divider' })) : null,
        showDeepSeek ? settingsCard() : React.createElement('div', { className: 'dshw_setCard' },
        ...
        activeProviderMode.kind === 'zai' ? React.createElement(PlanUsagePanel, { key: 'plans-compact', compact: true, provider: activeProviderMode.provider }) : null,
        showDeepSeek ? accountsSection() : null,
      ...
      if (floated) {
        if (minimized) {
          ...
          }, activeProviderMode.kind === 'zai' ? planTokenPercent : showDeepSeek ? (bal.total === null || bal.total === undefined ? '--' : fmtCurrency(bal.total, bal.currency)) : fmtTokens(thirdTokens))
        ...
        var floatPanel = React.createElement('div', {
          ...
          role: 'dialog',
          'aria-label': activeProviderMode.kind === 'zai' ? 'Z.ai 浮动套餐面板' : showDeepSeek ? 'DeepSeek 浮动钱包' : providerDisplayName(activeProviderMode) + ' 浮动用量面板'
        },
          ...
          showThird ? React.createElement(React.Fragment, null,
            thirdRows,
            React.createElement('div', { className: 'dshw_divider' })) : null,
          showDeepSeek ? visibilityControl : null,
          scaleControl,
          notifyControl,
          ...
          }, showDeepSeek ? '清除余额与 token' : '清除本会话 Token'),
          React.createElement('div', { className: 'dshw_divider' }),
          React.createElement(UsageHistoryPanel, { key: 'history-float', sessionId: sessionId }),
          activeProviderMode.kind === 'zai' ? React.createElement(PlanUsagePanel, { key: 'plans-float', compact: true, provider: activeProviderMode.provider }) : null,
          showDeepSeek ? accountsSection() : null,
          React.createElement('div', { style: { marginTop: '8px', textAlign: 'right', fontSize: '10px', color: 'var(--dsw-alias-label-secondary,var(--dsw-alias-label-tertiary,#667085))', userSelect: 'none' } }, 'Harness Control Center v' + WALLET_VERSION)
        )
