/* dsh-wallet client half: the composer-dock chip.
 * Loaded through the client module loader (CJS wrapper). All UI is React
 * via createElement; styles come from one injected <style> tag using the
 * product theme variables (--dsw-alias-*). */
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    if (typeof document !== 'undefined' && document.querySelector('style[data-plugin="dsh-wallet"]') === null) {
      var tag = document.createElement('style')
      tag.dataset.plugin = 'dsh-wallet'
      tag.textContent = css
    ...
    function WalletChip(props) {
      ...
      React.useEffect(function () {
        function onMove(event) {
          ...
          var x = Math.max(4, Math.min(window.innerWidth - 60, event.clientX - d.dx))
          var y = Math.max(4, Math.min(window.innerHeight - 40, event.clientY - d.dy))
          setFloatPos({ x: x, y: y })
        }
        function onUp() {
