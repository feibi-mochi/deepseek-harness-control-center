...
test('peak ring footer supports vertical layout, hidden recharge, and floating mode with reset', async () => {
  ...
  const source = readProjectFile('lib/client.js')
  assert.match(source, /requiredMoney > money\.clientWidth \+ 1/, 'the peak card reacts only when skin typography actually overflows')
  assert.match(source, /textWidth\(countdown\) > countdownRoom \+ 1/, 'countdown fit is measured independently from card width')
...
test('maid-atelier compatibility prevents the skin from forcing the wallet button to 30px', () => {
  ...
  assert.match(source, /\[data-composer-card\]\.dshw_modelMenuOpen\{backdrop-filter:none!important;-webkit-backdrop-filter:none!important\}/, 'opening the model picker disables the skin blur layer that can stall Edge repaints')
